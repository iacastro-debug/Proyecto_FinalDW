...
const props = defineProps<{ especialidad: any }>()
const form = reactive({ nombre: props.especialidad.nombre, descripcion: props.especialidad.descripcion, activo: props.especialidad.activo })
const loading = ref(false)
const submit = () => { loading.value = true; router.put(route('especialidades.update', props.especialidad.id), form, { onFinish: () => loading.value = false }) }
</script>
<template>
  <UDashboardPanel>
    <template #header><UDashboardNavbar title="Editar Especialidad"><template #leading><UDashboardSidebarCollapse /></template></UDashboardNavbar></template>
    <template #body>
      <div class="p-6 max-w-2xl mx-auto">
        <form @submit.prevent="submit" class="space-y-8">
          <UCard>
            ...
            <div class="space-y-4">
              ...
              <UFormGroup label="Estado">
                ...
                <p class="text-sm text-gray-500 mt-1">Si está inactiva, no aparecerá en los listados</p>
              </UFormGroup>
            </div>
          </UCard>
          ...
          <div class="flex gap-3 justify-end">
            ...
            <UButton type="submit" color="primary" label="Actualizar especialidad" :loading="loading" icon="i-lucide-save" />
          </div>
        </form>
      </div>
    </template>
