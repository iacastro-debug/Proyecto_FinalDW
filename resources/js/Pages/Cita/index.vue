...
const canCreate = computed(() => role.value === 'admin' || role.value === 'paciente')
...
const columns = computed(() => {
  ...
  if (canManage.value) cols.push({ accessorKey: 'acciones', header: 'Acciones' })
  return cols
})
  ...
  <UDashboardPanel>
    <template #header>
      <UDashboardNavbar title="Citas">
        ...
        <template #right>
          <UButton v-if="canCreate" label="Nueva" color="primary" icon="i-lucide-plus" :to="route('citas.create')" />
        </template>
      </UDashboardNavbar>
    </template>
    <template #body>
      <div class="p-4">
        <UTable :data="citas" :columns="columns">
          <template v-if="canManage" #cell-acciones="{ row }">
